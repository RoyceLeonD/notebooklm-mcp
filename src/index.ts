...
- File uploads: local PDF/PPT/PPTX files are supported through the authenticated
  browser UI when NotebookLM exposes \`input[type=file]\`; YouTube and Drive
  uploads are not implemented. If only an OS file-picker button is exposed,
  the tool returns an explicit incomplete/retryable UI result.
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        ...
        switch (name) {
          ...
          case "create_notebook_debug_session":
            result = await this.toolHandlers.handleCreateNotebookDebugSession(
              args as { notebook_url?: string; show_browser?: boolean }
            );
            break;
          case "inspect_notebook_page":
            result = await this.toolHandlers.handleInspectNotebookPage(
              args as { session_id: string; selectors?: string[]; screenshot?: boolean }
            );
            break;
          case "navigate_notebook_page":
            result = await this.toolHandlers.handleNavigateNotebookPage(
              args as { session_id: string; url: string }
            );
            break;
          case "click_notebook_element":
            result = await this.toolHandlers.handleClickNotebookElement(
              args as { session_id: string; selector: string; timeout_ms?: number }
            );
            break;
          case "type_notebook_element":
            result = await this.toolHandlers.handleTypeNotebookElement(
              args as { session_id: string; selector: string; text: string; timeout_ms?: number }
            );
            break;
